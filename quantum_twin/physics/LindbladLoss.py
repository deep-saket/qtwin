...
class LindbladLoss(PhysicsLoss):
    """Physics-informed loss enforcing the Lindblad master equation."""
    ...
    def __init__(
        ...
        self._operators = lindblad_ops.operators()
        self.logger.info("LindbladLoss ready with %d operators", len(self._operators))

    def _dissipator(self, rho: torch.Tensor) -> torch.Tensor:
        dissipator = torch.zeros_like(rho)
        for l_op in self._operators:
    ...
    def compute(self, model: torch.nn.Module, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        self.logger.debug("Computing Lindblad loss on batch size=%s", t.shape[0])
        ...
        residual = drho_dt_matrix + 1j * commutator - dissipator
        ...
        total = (
            self._weights.get("lindblad", 1.0) * physics_loss
        ...
        self.logger.debug(
            "Lindblad components physics=%.4f trace=%.4f hermitian=%.4f positivity=%.4f",
            physics_loss.item(),
            trace_penalty.item(),
