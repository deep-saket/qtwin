...
except ModuleNotFoundError:  # pragma: no cover - fallback when tensorboard is missing
...
class TensorboardWriter(BaseComponent):
    ...
    def __init__(self, log_dir: str) -> None:
        ...
        if SummaryWriter is None:
            self.logger.warning("TensorBoard not installed; logging disabled for %s", self._log_dir)
            self._writer = None
        else:
